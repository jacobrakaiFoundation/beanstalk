 ...
 *
 * Never enqueues notifications and never touches eligibility or classification.
 *
...
for (const id of ids) {
  ...
  try {
    ...
    console.log(`ok   ${id}  summary ${notice.summary.length} -> ${enriched.summary.length} chars  ${notice.sourceURL}`);
  } catch (error) {
    failed += 1;
