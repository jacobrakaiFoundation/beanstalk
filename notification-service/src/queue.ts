...
import type { ApnsEnvironment, DeviceStore, PushIdentifierKind, PushProvider } from "./devices.js";
    ...
    const due = this.database.connection
      .prepare(`
        SELECT q.id, q.notice_id, q.device_id, q.matched_term, q.matched_field, q.attempts,
               n.publication_date, d.device_token, d.environment, d.provider, d.identifier_kind
    ...
    for (const row of due) {
      ...
      if (result.kind === "success") {
        ...
        const disable = this.database.connection.transaction(() => {
          ...
          this.failPermanently(row.id, attempt, code, row.provider, now);
          this.database.connection
        ...
        this.failPermanently(row.id, attempt, safeErrorCode(result.code), row.provider, now);
      }
      processed += 1;
    }
    return processed;
